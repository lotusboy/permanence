...
| `design/` | detailed, forward-looking design docs for work substantial enough to need one — not yet built |
...
> Fully automated on **Claude Code**, **Antigravity**, and **Devin** — you carry nothing (the `/perma-*` commands, plus hooks that orient the session and force the read before the model answers). **Cursor** is fully automated too, in interactive use — the one gap is headless (`-p`) sessions, which get no automatic standing-instruction coverage. Works with other AI tools too, anything reading the `AGENTS.md` standard; what *you* have to do by hand depends on what the tool supports — for one that only reads a global config file, that's saying *"good morning Permanence"* to orient it, since nothing injects that automatically. See [docs/TOOL-SUPPORT.md](./docs/TOOL-SUPPORT.md), which also covers **Claude Desktop's Code and Cowork tabs specifically** (including what IT/Ops needs to grant). Using something with no file/shell access at all, like a plain chat interface? The manual pointer in that doc still works for reading — but Permanence needs real command execution (`git`, `mkdir`), not just chat. `SPEC.md` separates the harness-independent contract from any binding. Built on the **axis-engineering** methodology (a separate, public companion — optional).
